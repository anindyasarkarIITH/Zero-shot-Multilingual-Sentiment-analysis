...
for num_epoch in range(num_epochs):
	...
	for i_batch, sample_batched in enumerate(data_loader):
		...
		seq_lengths = [values.tolist() for values in sample_batched[2]]
		
		
		batch_in = sample_batched[0]
		
		sent_lengths = sample_batched[1].tolist()
		...
		## sorted_seq_length --> [NUMBER of words in 1st sentence,NUMBER of words in 1st sentence,...,25th]
                ## sent_length_tensor --> no. of sent in review..list
		y_pred,_ = sent_net(sent_vect,sorted_seq_length,sorted_idx,hidden_size,max_length_word,batch_size,sent_length_tensor)
